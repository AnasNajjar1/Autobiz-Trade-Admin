...
const SubMenuLayout = ({resources, onMenuClick}) => {
    ...
    const subMenu = ['offline', 'onSale', 'auctionFinished', 'auctionFailed', 'purchasedImmediately', 'sold']
    ...
    return (
        <div>
            ...
            {resources.map(item => {
                if(!subMenu.includes(item.name) && item.name !== 'vehicles' && !facadeList.includes(item.name)) {
                    return menuLayout(item, onMenuClick) 
                }
            })}
        </div>
